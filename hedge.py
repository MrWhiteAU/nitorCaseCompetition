...
def load_and_prep_data():
    print("Loading data for the HEDGE model...")
    ...
    # THE HEDGE: Re-introducing 2023 data to learn extreme volatility
    train = train[train['delivery_start'] >= '2023-01-01'].copy()
    
    # THE HEDGE: Relaxing the clip to catch massive Private LB spikes
    train['target'] = np.clip(train['target'], -500, 3000)
...
def run_pipeline():
    ...
    if raw_weights.sum() == 0:
        blend_weights = np.array([0.50, 0.50])
    else:
        blend_weights = raw_weights / raw_weights.sum()
        
    print(f"Optimal Learned Weights (Intercept Disabled):")
    print(f"XGBoost:  {blend_weights[0]*100:.2f}%")
    print(f"CatBoost: {blend_weights[1]*100:.2f}%")

    oof_preds_blend = (
        (blend_weights[0] * oof_preds_xgb) + 
        (blend_weights[1] * oof_preds_cat)
    )
    
    final_test_preds = (
        (blend_weights[0] * test_preds_xgb) + 
        (blend_weights[1] * test_preds_cat)
    )
    ...
    sub.to_csv('hedge_submission.csv', index=False, lineterminator='\n')
    print("✅ File 'hedge_submission.csv' generated successfully!")
